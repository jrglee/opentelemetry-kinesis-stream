...
fmt-check:
	@out="$$(gofumpt -l $$(git ls-files '*.go'))"; if [ -n "$$out" ]; then echo "gofumpt needed on:"; echo "$$out"; exit 1; fi
...
# from distro/builder-config.yaml (ADR-0023). OCB_VERSION must match the
# collector version pinned in go.mod and builder-config.yaml.
OCB_VERSION := v0.154.0
OCB := go run go.opentelemetry.io/collector/cmd/builder@$(OCB_VERSION)

collector:
	CGO_ENABLED=0 $(OCB) --config distro/builder-config.yaml
	@mkdir -p bin && cp distro/_build/otelcol-kinesis bin/otelcol-kinesis

docker:
	docker build -f distro/Dockerfile --build-arg OCB_VERSION=$(OCB_VERSION) -t otelcol-kinesis:dev .
...
clean:
	rm -rf bin/ coverage.out compose/shared distro/_build
	go clean ./...
