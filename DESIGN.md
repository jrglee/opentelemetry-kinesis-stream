...
4. Only OTLP proto/JSON, Jaeger proto, and Zipkin encodings are available. OTel-Arrow looked like an attractive fourth option on paper — clear throughput advantages for large telemetry batches and a maturing reference implementation in `open-telemetry/otel-arrow` — but in practice OTAP's compression win depends on a stateful, long-lived gRPC stream with cross-batch dictionary deltas. Kinesis's store-and-forward, per-record delivery model forfeits that statefulness; what remains is per-record schema overhead without the payoff. We prototyped it, benchmarked it, and removed it: see [`benchmark.md`](benchmark.md) and [ADR-0020](docs/adr/0020-remove-otel-arrow-encoding.md).
...
Microbatching trades per-record granularity for better amortization of fixed costs (compression headers, `PutRecords` entry overhead, Kinesis per-record billing) when telemetry is sparse, without changing the wire shape on the receiving side — a record decodes to one or more resources via the same encoding.
...
- Treat encoding as an open registry. The registry shape matters more than which encoding shipped first. As built, it carries `otlp_proto` and `otlp_json`. An `otel_arrow` implementation lived briefly behind the same registry and was removed after benchmarking confirmed the design mismatch with store-and-forward delivery — see [ADR-0020](docs/adr/0020-remove-otel-arrow-encoding.md) and [`benchmark.md`](benchmark.md).
...
- **Versioning follows semver.** Minor releases are backward-compatible at the wire, configuration, and checkpoint-schema layers. Major releases may break any of these and must document the migration.
